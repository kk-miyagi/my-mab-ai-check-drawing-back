...
async def update_label(request: Request):
    ...
    # TODO: group_idの対応次第で修正
    req_user = req_status.user
    req_epic = req_status.epic
    req_group_id = req_status.group_id
    req_group_status = req_status.group_status
    req_other = req_status.other
    req_operations = req_status.operations
    req_operation = req_operations[0]["operation"]
    req_operation_id = req_operations[0]["operation_id"]
    req_operation_status = req_operations[0]["status"]

    # TODO: ラベル付与していない図面の保存先の指定
    input_dir = f"./multi-fileupload/{req_user}_{req_epic}_{req_group_id}_{req_operation_id}"

    # TODO: ラベル付与後の図面の保存先の指定
    output_dir = f"./update-label-output/{req_user}_{req_epic}_{req_group_id}_{req_operation_id}"
    ...
    # TODO: ラベル付与の編集(どのようなステータス管理にするか次第)
    # 一旦、DOINGのときにラベル付与する想定とする
    match req_group_status:
        case Status.START:
            logger.log(
                req_status,
                AppLogger.INFO,
        ...
        case Status.DOING:
            ...
            rects_dict = req_other["rects"]
            info_dict = req_other["info"]
            ...
            ordered_matches = [
                {"id": str(k), "rect": v}
                for k, v in req_other["rects"].items()
            ]
            ...
            with open(csv_path, "w", encoding="utf-8") as f:
                ...
                for key, info in req_other["info"].items():
