...
export const InputFiles: React.FC<InputFilesProps> = ({
  ...
  const appendFiles = (files: readonly File[]) => {
    ...
    const nextPickedItems = toUploadFileItems(files);
    const baseItems = [...items];
    const mergedMap = new Map<string, UploadFileItem>();

    baseItems.forEach((item) => mergedMap.set(item.id, item));
    nextPickedItems.forEach((item) => {
