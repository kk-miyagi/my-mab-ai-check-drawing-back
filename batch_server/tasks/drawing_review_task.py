...
def run(params: dict) -> None:
    """Worker entrypoint. Required keys: excel_dir, pdf_dir, output_dir."""
    _run_review(params['excel_dir'], params['pdf_dir'], params['output_dir'])


def _run_review(excel_dir_path: str, pdf_dir_path: str, output_dir_path: str):
    if not os.path.isdir(output_dir_path):
        os.makedirs(output_dir_path)
        print(f"{output_dir_path}を作成しました")

    output_dir = Path(output_dir_path)

    excel_dir = Path(excel_dir_path)
    ...
    image_files = [pdf_to_jpeg(file) for file in pdf_files]  # noqa: F841
...
if __name__ == "__main__":
    ...
    parse.add_argument("--excel-dir", type=str, help="Excelファイルのパス")
    parse.add_argument("--pdf-dir", type=str, help="PDFの図面のパス")
    parse.add_argument("--output-dir", type=str, help="実行結果出力先ディレクトリ")
    args = parse.parse_args()
    _run_review(args.excel_dir, args.pdf_dir, args.output_dir)
    sys.exit(0)
