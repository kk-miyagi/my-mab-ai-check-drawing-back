...
from batch_server.tasks.utils.dimension_models import AggregatedToken, OCRParagraph, OCRToken
...
from batch_server.tasks.utils.test_vision_ocr import (
    _extract_paragraphs_and_tokens,
    perform_document_text_detection,
)
from batch_server.tasks.utils.template_match_locator import locate_tiles
...
def run(params: dict) -> None:
    """Entrypoint used by the batch worker.

    Expected keys: in_dir, image, out_dir.
    Optional: box_on (default True), tile_dir, tile_tolerance,
    tile_max_results, tile_region_padding.
    """
    dir_name = params['in_dir']
    if dir_name and not dir_name.endswith((os.sep, "/")):
        dir_name = dir_name + os.sep
    file_list: Dict[str, str] = {
        "MAB_picture1": params['image'],
        "MAB_picture2": params['image'],
    }
    demission_group(
        dir_name,
        file_list,
        tile_dir=params.get('tile_dir', 'cut/'),
        tile_tolerance=params.get('tile_tolerance', 1e-2),
        tile_max_results=params.get('tile_max_results', 1),
        tile_region_padding=params.get('tile_region_padding', 30),
        output_dir=params['out_dir'],
        box_on=params.get('box_on', True),
    )
