...
def run(params: dict) -> None:
    """Worker entrypoint. Required keys: base_image_path, target_image_path,
    base_cut_dir, target_cut_dir, out_dir. Optional: combinations (dict)."""
    _run_compare(
        params['base_image_path'],
        params['target_image_path'],
        params['base_cut_dir'],
        params['target_cut_dir'],
        params['out_dir'],
        params.get('combinations'),
    )
...
def _run_compare(
    ...
    if combinations_override is not None:
        combinations = combinations_override if isinstance(
                combinations_override, dict) else (
                    json.loads(combinations_override)
                    if isinstance(combinations_override, str)
                    and combinations_override
                    else {})
    else:
...
if __name__ == "__main__":
    ...
    parse.add_argument("--base-image-path", type=str)
    parse.add_argument("--target-image-path", type=str)
    parse.add_argument("--base-cut-dir", type=str)
    parse.add_argument("--target-cut-dir", type=str)
    parse.add_argument("--out-dir", type=str)
